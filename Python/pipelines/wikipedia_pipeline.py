...
def transform_wikipedia_data(**kwargs):
    ...
    data = kwargs['ti'].xcom_pull(key='rows', task_ids='extract_data_from_wikipedia_data')
...
def write_wikipedia_data(**kwargs):
    ...
    file_name = ('stadium cleaned '+str(datetime.now().date())
                 +'_'+ str(datetime.now().time()).replace(':','_')+'.csv')

    data.to_csv('abfs://bigdataproject@bigdataproject.dfs.core.windows.net/formatted/wikipedia/Stadiums/20240615/'+file_name,
                storage_options={
                    'account_key': ''
                    #Account Key retiré par soucis de sécurité
                                 }, index=False)
